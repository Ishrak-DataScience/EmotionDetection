...
from transformers import ElectraTokenizer, ElectraModel, ElectraForSequenceClassification, get_scheduler
...
class ElectraEmotionClassifier(nn.Module):
    def __init__(self, electra_model, num_labels):
        super(ElectraEmotionClassifier, self).__init__()
        self.electra = electra_model
        self.fc = nn.Linear(electra_model.config.hidden_size, num_labels)

    def forward(self, input_ids, attention_mask):
        outputs = self.electra(input_ids=input_ids, attention_mask=attention_mask)
        logits = self.fc(outputs.last_hidden_state[:, 0, :])  # Use CLS token
        return logits
...
electra_model = ElectraModel.from_pretrained('google/electra-large-discriminator')
model = ElectraEmotionClassifier(electra_model, num_labels=len(emotion_columns))
...
output_path = "/home/rabo074f/LLM_Project_Ramy/LLM/test_predictions_electra_large.csv"
