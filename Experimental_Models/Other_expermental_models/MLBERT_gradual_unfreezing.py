...
# Define the BiLSTM classifier for multilingual BERT
class MultilingualBERTBiLSTMClassifier(nn.Module):
    def __init__(self, bert_model, hidden_dim, num_labels):
        super(MultilingualBERTBiLSTMClassifier, self).__init__()
...
# Initialize the BiLSTM model with multilingual BERT
bert_model = BertModel.from_pretrained('bert-base-multilingual-cased')
model = MultilingualBERTBiLSTMClassifier(bert_model, hidden_dim=256, num_labels=len(emotion_columns))
...
# Freeze all layers of the BERT model initially
...
def unfreeze_layers(model, num_layers_to_unfreeze):
    ...
    total_layers = len(list(model.bert.encoder.layer))
    layers_to_unfreeze = list(model.bert.encoder.layer)[-num_layers_to_unfreeze:]
